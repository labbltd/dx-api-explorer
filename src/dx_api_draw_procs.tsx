                    ...
                    <div className="control">
                        {
                            ...
                            <textarea className="textarea"
                                ...
                                {app.resources.fields.get(component.key)!.data}
                            </textarea>
                        }
                        {
                            (
...
function Draw_debug_fields(props: { app: app_context_t }) {
    ...
    return <div className="columns">
        <table className="table column">
            ...
            <tbody>
                {Array.from(app.resources.fields.keys()).map(f => <tr key={f} onClick={() => setSelected(f)}>
    ...
    return <div className="content columns">
        <div className="column">
            ...
            <table className="table">
                <thead><tr><th>Key</th><th>Value</th></tr></thead>
                <tbody>
                    {Object.keys(operator).map(key => <tr key={key}><td>{key}</td><td>{typeof operator[key] === 'number' ? new Date(+(operator[key] + '000')).toLocaleTimeString() : operator[key]}</td></tr>)}
                </tbody>
            </table>
            <button type="button" className="button" onClick={() => {
